...
import { generateImage, imagesEnabled, IMAGE_ASPECTS, ImageError } from "@/lib/ai/image";
...
export async function POST(request: Request) {
  ...
  const { prompt, aspect, alt, clientId } = parsed.data;

  try {
    const { bytes, mime } = await generateImage({ prompt, aspect, signal: request.signal });
