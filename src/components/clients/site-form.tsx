...
export function SiteForm({ clientId, site }: { clientId: string; site?: SiteFormValues }) {
  ...
  return (
    <form action={formAction} onSubmit={onSubmit} noValidate className="space-y-6">
      ...
      <details
        className="group rounded-[var(--radius-panel)] border border-line bg-surface open:bg-transparent open:border-transparent"
        open={platform !== "api" || Boolean(state && !state.ok)}
      >
        <summary className="flex min-h-12 cursor-pointer list-none items-center justify-between gap-3 px-5 py-3 text-sm font-medium text-ink group-open:mb-4 group-open:px-0">
          <span>
            Configurações avançadas
            <span className="ml-2 font-normal text-muted">WordPress, webhook, autor padrão. Sites feitos pela OutBox não precisam.</span>
          </span>
          ...
          <div className="mt-6 border-t border-line pt-6">
            ...
            {platform === "webhook" ? (
              ...
              <p className="text-sm text-muted">
                Automático: o site do cliente busca os artigos pelo próprio domínio e é avisado a cada publicação. Não há nada para configurar aqui.
                <input type="hidden" name="webhook_url" value={apiWebhookValue === revalidateUrl ? "" : apiWebhookValue} />
              </p>
            ) : (
              <Field
